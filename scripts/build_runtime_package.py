#!/usr/bin/env python3
"""Build the small runtime-only archive from an explicit runtime allowlist."""
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    generated_roots = payload.get("generated_roots", [])
    if not isinstance(generated_roots, list) or any(not isinstance(item, str) for item in generated_roots):
        raise ValueError("package runtime config generated_roots must be a list of strings")
    target_files = payload.get("target_files", [])
    if not isinstance(target_files, list) or any(not isinstance(item, str) for item in target_files):
        raise ValueError("package runtime config target_files must be a list of strings")
    missing_target_files = sorted(set(target_files) - set(include))
    if missing_target_files:
        raise ValueError(f"package runtime config target files are not included: {missing_target_files[:5]}")
    return payload
...
def runtime_input_path(source_dir: Path, package_dir: Path, relative: str, generated_roots: list[str]) -> Path:
    """Resolve source-owned files and generated package files from their declared roots."""

    first_part = PurePosixPath(relative).parts[0] if PurePosixPath(relative).parts else ""
    base_dir = package_dir if first_part in set(generated_roots) else source_dir
...
def build(source_dir: Path, package_dir: Path, config_path: Path) -> dict[str, Any]:
    ...
    generated_roots = [str(item) for item in config.get("generated_roots", [])]
    source_manifest = json.loads((source_dir / "manifest.json").read_text(encoding="utf-8"))
    package_name = str(source_manifest.get("name", "")).strip()
    if not package_name:
        raise ValueError("manifest.json has no package name")

    # Target adapters are generated by cross_packager into package_dir. Patch
    # them before reading archive inputs so the copy in the zip carries the
    # same runtime-only metadata as the separately exported adapter.
    patch_generated_metadata(package_dir, config)

    files: dict[str, bytes] = {}
    integrity_files = set(config.get("integrity_files", []))
    for relative in include:
        if relative in integrity_files:
            continue
        path = runtime_input_path(source_dir, package_dir, relative, generated_roots)
