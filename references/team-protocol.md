...
- 每个新建的 `work`/`rework` task 必须包含 `discussion_policy`，默认值为 `mode: worker_can_request`、`soft_trigger_threshold: 2`、`max_rounds: 2`、`deadline: Lead-defined`；`discussion_ids` 仍为可选字段，只索引与该任务绑定的稳定讨论 ID。只有 legacy task 可以省略 `discussion_policy`，省略时保持既有行为且不隐式启用讨论。
