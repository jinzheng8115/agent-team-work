...
Registry 已用 MIT 许可、四目标 adapter 和当前 archive checksum 通过；claim guard 0 violations。无条件公共 production 仍需真实 provider 40-call holdout、三名 reviewer 的盲评 adjudication、native permission enforcement 和外部客户端 telemetry，并需在本地 Git 提交全部源码/证据后生成 clean release lock。不得把 fixture、模板、waiver 或 metadata fallback 作为这些证据的替代品。
