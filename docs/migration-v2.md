...
Legacy tasks without `discussion_policy` retain the current serial behavior and do not gain worker-to-worker discussion implicitly. Newly created tasks default to `mode: worker_can_request`, `soft_trigger_threshold: 2`, `max_rounds: 2`, and a Lead-defined deadline. Adding the policy does not migrate old task identity, relax the Lead-only `team.json`/`tasks.json` writer rule, or allow a discussion decision to bypass normal task acceptance and stage dispatch.
